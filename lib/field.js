...
Field._addCheck = function(checkSettings) {
  this.checks.push(_.extend({
    type: null,
    args: null,
    message: null,
    validation: null
...
Field.specify = function(validation) {
...
Field.extend = function(extensions) {
  ...
  // merge checks
  SubField.checks = _.clone(this.checks);
  (extensions.checks || []).forEach(function(check) {
    if (check.type) {
      SubField.type(check.type, check.args || [], check.message || '');
    } else {
      SubField.specify(check.validation);
    }
  });
...
Field.prototype.validate = function(input, callback) {
  ...
  async.eachSeries(this.constructor.checks, function(check, nextLoop) {
    var type = check.type;
    var args = check.args;
    ...
    if (
      ...
      self.constructor.shouldCheckAll === false && invalidResults.length > 0
    ) {
      nextLoop();
    } else if (validation) {
